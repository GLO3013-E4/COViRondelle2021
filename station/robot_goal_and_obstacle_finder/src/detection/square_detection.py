...
class SquareDetection(ObjectDetection):
    ...
    def __init__(self, image):
        ObjectDetection.__init__(self, image, "square", 440, 550)
    ...
    def generate_four_corners(self, x_position, y_position, width, height):
        corner_a = Point(x_position + width, y_position).get_coordinates()
        corner_b = Point(x_position + width, y_position + height).get_coordinates()
        corner_c = Point(x_position, y_position + height).get_coordinates()
        corner_d = Point(x_position, y_position).get_coordinates()
    ...
    def _get_contours(self, image_mask):
        # sauce = cv2.findContours(image_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[-2:]
        contours, hierarchy = cv2.findContours(image_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[-2:]
